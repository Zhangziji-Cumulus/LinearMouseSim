...
class ParameterPanel(tk.Frame):
    ...
    def _build_content_area(self):
        ...
        self._canvas = tk.Canvas(self, bg=Theme.SURFACE, highlightthickness=0)
        self._scrollbar = tk.Scrollbar(self, orient=tk.VERTICAL, command=self._canvas.yview)
        self._content_frame = tk.Frame(self._canvas, bg=Theme.SURFACE)
        ...
        self._scrollbar.pack(side=tk.RIGHT, fill=tk.Y, pady=8, padx=(0, 12))

        # 绑定鼠标滚轮 - 使用 Enter/Leave 事件管理
        self._canvas.bind('<Enter>', self._bind_mousewheel)
        self._canvas.bind('<Leave>', self._unbind_mousewheel)

        # 让 content_frame 的宽度跟随 canvas
    ...
    def _bind_mousewheel(self, event):
        """鼠标进入时绑定滚轮事件"""
        self._canvas.bind_all('<MouseWheel>', self._on_canvas_scroll)
    ...
    def set_parameters(self, params):
        ...
        if 'center_enabled' in params and hasattr(self, '_center_enabled_var'):
            self._center_enabled_var.set(bool(params['center_enabled']))
        if 'center_mode' in params and hasattr(self, '_center_mode_var'):
            mode_map = {0: '延迟', 1: '立即', 2: '持续'}
            self._center_mode_var.set(mode_map.get(params['center_mode'], '立即'))
        if 'center_speed_mode' in params and hasattr(self, '_center_speed_mode_var'):
            speed_mode_map = {0: '固定', 1: '动态'}
            self._center_speed_mode_var.set(speed_mode_map.get(params['center_speed_mode'], '固定'))
